...
results <- runAnalysis("msaGaugeRRnonrep", "datasets/msaGaugeRRNested/msaGaugeRRNested_long.csv", options, makeTests = T)

test_that("LF3 - Gauge r&R (non-replicable) report plot matches", {
...
options$measurementsWideFormat <- "Result"
options$tolerance <- TRUE
options$toleranceValue <- 15
options$rChart <- TRUE
options$xBarChart <- TRUE
